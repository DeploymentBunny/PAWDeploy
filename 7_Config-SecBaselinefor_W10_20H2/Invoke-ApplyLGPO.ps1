...
& robocopy $SOURCEROOT c:\SECBL /e /s
$Zips = Get-ChildItem -Path c:\SECBL -Filter *.zip
foreach($Zip in $Zips){
    Expand-Archive -Path $zip.fullname -DestinationPath c:\SECBL\$($zip.shortname) -Force
    Remove-Item -Path $zip.fullname -Force
}

$item = Get-ChildItem C:\SECBL -Filter lgpo.exe -Recurse
Copy-Item -Path $item.FullName C:\SECBL\Local_Script\Tools
#C:\SECBL\Local_Script\BaselineLocalInstall.ps1 -Win10NonDomainJoined
